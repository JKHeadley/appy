...
internals.updateGroup = function (newGroup, oldGroup) {
  ...
  promises.push(internals.updateGroupPermissions(newGroup._id, newPermissions, oldPermissions))
...
internals.updateGroupUsers = function (groupId, newUsers, oldUsers) {
  ...
  let usersToAdd = newUsers.filter((newUser) => {
    return !oldUsers.find((oldUser) => { return oldUser.user._id === newUser.user._id })
  }).map((user) => { return user.user._id })

  let usersToRemove = oldUsers.filter((oldUser) => {
    return !newUsers.find((newUser) => { return oldUser.user._id === newUser.user._id })
  }).map((user) => { return user.user._id })

  let promise = _.isEmpty(usersToAdd) ? Promise.resolve() : vm.$groupRepository.addManyUsers(groupId, usersToAdd)

  return promise
    .then(() => {
      return _.isEmpty(usersToRemove) ? Promise.resolve() : vm.$groupRepository.removeManyUsers(groupId, usersToRemove)
    })
}

internals.updateGroupPermissions = function (groupId, newPermissions, oldPermissions) {
  ...
  let promise = _.isEmpty(permissionsToAdd) ? Promise.resolve() : vm.$groupRepository.addManyPermissions(groupId, permissionsToAdd)

  return promise
    .then(() => {
      return _.isEmpty(permissionsToRemove) ? Promise.resolve() : vm.$groupRepository.removeManyPermissions(groupId, permissionsToRemove)
    })
}
