...
internals.createPermission = function (permission) {
  ...
  const users = (permission.users || []).concat([]).map((user) => { return user.user._id })
  ...
  const promises = []
  return vm.$permissionRepository.create(permission)
    .then((result) => {
      permission = result.data
      if (!_.isEmpty(users)) {
        promises.push(vm.$permissionRepository.addManyUsers(permission._id, users))
      }
      if (!_.isEmpty(groups)) {
        promises.push(vm.$permissionRepository.addManyGroups(permission._id, groups))
      }
...
internals.updatePermission = function (newPermission, oldPermission) {
  ...
  const promises = []
  promises.push(vm.$permissionRepository.update(newPermission._id, newPermission))
  promises.push(internals.updatePermissionGroups(newPermission._id, newGroups, oldGroups))
  promises.push(internals.updatePermissionUsers(newPermission._id, newUsers, oldUsers))
...
internals.updatePermissionUsers = function (permissionId, newUsers, oldUsers) {
  ...
  let promise = _.isEmpty(usersToAdd) ? Promise.resolve() : vm.$permissionRepository.addManyUsers(permissionId, usersToAdd)

  return promise
    .then(() => {
      return _.isEmpty(usersToRemove) ? Promise.resolve() : vm.$permissionRepository.removeManyUsers(permissionId, usersToRemove)
    })
}

internals.updatePermissionGroups = function (permissionId, newGroups, oldGroups) {
  ...
  }).map((group) => { return { childId: group.group._id, state: group.state } })
  ...
  let promise = _.isEmpty(groupsToAdd) ? Promise.resolve() : vm.$permissionRepository.addManyGroups(permissionId, groupsToAdd)

  return promise
    .then(() => {
      return _.isEmpty(groupsToRemove) ? Promise.resolve() : vm.$permissionRepository.removeManyGroups(permissionId, groupsToRemove)
    })
}
